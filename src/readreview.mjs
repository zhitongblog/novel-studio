...
export function buildReadFixInstruction(items) {
  ...
  return [
    ...
    ...(items.some(i => i.kind === '篇幅') ? [
      ...
      // 【天花板】只给下限，模型就会冲天花板——这是同一个坑今晚第三次。
      // 实证：第 22 章只缺 76 字，补完冲到 3997（上限 3600），等于把"不足"换成了"超标"。
      '【篇幅的天花板】目标区间是 3000–3600 字，**补完不得超过 3600**。'
      + '补到刚过 3000 就停手，不要越补越多；也不要为了腾地方去删原有的情节'
      + '——原文能留就留，这是已经发出去给读者看过的章',
    ] : []),
