...
const DEFAULTS = {
  ...
  gemini: {                          // Google Gemini / Imagen（用于 AI 生成封面底图、把中文题材翻成英文画面提示）
    ...
    textModel: 'gemini-2.5-flash',   // 把中文题材润色成英文出图提示词
  },
  ...
  autopilot: {
    ...
    continueText: '继续下一批。动笔前先重建上下文：读 continuity_ledger.md，再读最近 2 章正文与本卷 outlines/ 中对应章号段的分章大纲；据此确认当前最新章号、主角处境、未回收伏笔、欠债与伤势。然后严格按 longform-webnovel-writer 标准写下一批正文（仅正文、单章字数达标）。写完务必：①把新章登记进 chapter_index.md；②更新 continuity_ledger.md（人物现状/已知信息、未回收伏笔、欠债与承诺、伤势、关键物件去向、时间线锚点）；③做常规批次自检。',
    fullCheckEvery: 5,               // 每 N 次续写插入一次"全文逻辑自检"（0=关闭）
    fullCheckText: '现在做一次【全文逻辑自检】，不要只看最近一批：通读 novel_bible.md、全部 outlines/、chapter_index.md 与 continuity_ledger.md（若无则新建），并抽查关键章节，核查全书的——时间线先后、人物弧光与性格一致性、力量/设定体系规则是否自洽、伏笔的铺设与回收、未决线索与“待查”项、事实/关系/伤势/物件/欠债的前后矛盾、章名与命名一致性、与各卷大纲的偏离。把发现按【硬伤/隐患/待办】三档写入 reviews/全文逻辑自检-至<最新全局章号>.md，并更新 continuity_ledger.md。就地修正“硬伤”级矛盾（保持已写剧情框架与结局，只改必要处），无法当场修的明确记录为待办。完成后再继续按 longform-webnovel-writer 标准写下一批并做常规批次自检。',
