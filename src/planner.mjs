...
export function buildFreehandKickoffInstruction(book, theme, words, characters) {
  ...
  const s = `你是资深网文主编。现在为《${book.title}》做【探索式立项——只定"怎么写"，不定"写什么"】。` +
    ...
    `句式与段落形态（段落多长、换行密度、对话与叙述配比、严禁逐句换行）、单章结构（开头怎么起、中段怎么推、章末钩子怎么抛）、` +
    `单章 ${lo}–${hi} 字、实锚密度（每 250–400 字一个可见实锚：物件/身体感觉/具体动作/专有名词与数目）、` +
    `对话怎么写（潜台词、人物口吻如何区分）、爽点与节奏的做法、` +
    `命名口味基线（人名/地名什么味道——这条是给你以后【临时起配角名】时照着起的，务必写具体）、` +
    `以及【反 AI 味禁令】（禁解释性套话"这不是…而是/这意味着"、禁翻译腔现代词、禁段尾点题升华、禁句长均匀、禁重复句式；` +
    `「……」一章不超过 5 次、正常成段）。` +
